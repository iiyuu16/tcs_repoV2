...
public class wormPlyrScan : MonoBehaviour
{
    public static wormPlyrScan instance;
    ...
    void Start()
    {
        ...
        plyrWorm = FindObjectOfType<plyrWorm>();
    }

    void Update()
    {
        if (!plyrWorm.isStunned && Input.GetKeyDown(Scan) && Time.time >= nextScanTime)
        {
            scanner.Play();
            sfx.scanSFX();
            StartExpanding();
